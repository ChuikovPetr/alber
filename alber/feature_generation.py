...
def book_preprocessor(df):
    ...
    df['bid_ask_w_spread1'] = (df['ask_price1'] * df['ask_size1']) / (df['bid_price1'] * df['bid_size1']) - 1
    df['bid_ask_w_spread2'] = (df['ask_price2'] * df['ask_size2']) / (df['bid_price2'] * df['bid_size2']) - 1
    df['bid_w_spread'] = (df['bid_price1'] * df['bid_size1']) / (df['bid_price2'] * df['bid_size2']) - 1
    df['bid_w_ask'] = (df['ask_price1'] * df['ask_size1']) / (df['ask_price2'] * df['ask_size2']) - 1
...
# +
def get_zscore(df: pd.DataFrame, span: int, field: str, group_fields: List[str]) -> pd.DataFrame:
    res_i_list = [i for i in range(len(group_fields))]
    
    
    df['mean'] = df\
    .groupby(group_fields)[field]\
...
def get_trend_features(
    ...
    for n_1, n_2 in np.array(get_grid(spans), dtype=int):
        df[f'rel_{field}_{n_1}_{n_2}'] = \
        df.apply(lambda x: x[f'ma_{n_1}'] / x[f'ma_{n_2}'], axis=1)
...
def get_stoch(df: pd.DataFrame, span: int, field: str, group_fields: List[str]) -> pd.DataFrame:
    res_i_list = [i for i in range(len(group_fields))]
    ...
    df['stoch'] = df.apply(lambda x: 100 * (x[field] - x['MIN']) / (0.0000001 + x['MAX'] - x['MIN']), axis=1)
    
    df[f'stoch_k_{field}_{span[0]}_{span[1]}'] = df\
    .groupby(group_fields)['stoch']\
    .rolling(span[1])\
    .apply(lambda x: np.mean(x))\
    .reset_index(res_i_list, drop=True)
    
    df[f'stoch_d_{field}_{span[0]}_{span[2]}'] = df\
    .groupby(group_fields)[f'stoch_k_{field}_{span[0]}_{span[1]}']\
    .rolling(span[2])\
    .apply(lambda x: np.mean(x))\
    .reset_index(res_i_list, drop=True)
    
    df[f'rel_stoch_{field}_{span[0]}_{span[1]}_{span[2]}'] = df\
    .apply(lambda x: x[f'stoch_k_{field}_{span[0]}_{span[1]}'] / (x[f'stoch_d_{field}_{span[0]}_{span[2]}'] + 0.0001), axis=1)
