...
Далее приведены результаты 3 моделей на out-of-time. Само собой нельзя сравнивать модели на тесте и после этого выбирать лучшую, так как мы просто подстроимся под тест. Но, специалисты будут проверять модель на своем out-of-time. Тогда можем считать мою проверку на out-of-time предтестом.
...
| ------------- | ------------- |
| model_mf_15f  | 0.0193  |
| Baseline, установленный в ТЗ | 0.01  |
| baseline_1f  | 0.007  |
| model_of_29f  | 0.004  |
...
Структура:
1. Папка <strong>alber</strong> с необходимыми модулями:
   ...
   1.4. <strong>train.py</strong> - содержит функции для подготовки витрины для обучения модели, обучения модели и проверки обученной модели на out-of-time.

   1.5. <strong>forecasting.py</strong> - содержит функции для подготовки витрины, содержащей только необходимые признаки, а также функции для загрузки модели и создания файла forescast.h5 с результатами предсказания (имеет ту же структуру, что и файл result.h5, с тем лишь отличием, что вместо поля Res, он содержит поле Score).

   1.6. utils.py and one_factor.py - вспомогательные модули, которые были необходимы непосредственно для построения модели.

2. Папка <strong>notebooks</strong> с следующими ноутбуками:

   2.1. Result - Fitting.ipynb - результат выполненного задания (ноутбук для обучения). Подробнее смотри далее.
   2.2. Result - Forecasting.ipynb - результат выполненного задания (ноутбук для получения предсказаний). Подробнее смотри далее.
   2.3. Остальные ноутбуки, необходимые непосредственно для построения модели.
...
Важно! В файле обучается 3 модели: baseline_1f, model_of_29f, model_mf_15f, если вы хотите обучить только одну просто не запускайте соответствующие клетки.

Тоже важно! Поскольку в файле обучались различные модели с разным набором фичей, витрина с признаками содержит в себе все признаки, а потому для большого датасета будет считаться долго (чтобы узнать сколько именно см. пункт [Features](#features)).
...
Второй скрипт - notebooks/Result - Forecasting.ipynb - принимает в качестве параметра - data_path - путь к файлу наподобии data.h5, затем рассчитывает только необходимые признаки на основе этих данных, загружает модель и сохраняет результат в файл score_path (тоже параметр), имеющий ту же структуру, что и файл result.h5, с тем лишь отличием, что вместо поля Res, он содержит поле Score.
...
Важно! В данных были дубли по временным меткам для биржевого стакана и для наблюдений (дубли для трейдов - нормальное дело, у меня даже есть признаки на основе количества трейдов в конкретную единицу времени). Была произведена дедубликация, просьба учитывать это при проверке на новом out-of-time.
...
Данные оказались слишком шумные для точной настройки параметров модели - не получилось стат. значимо увеличить метрику, изменяя параметры lightgbm. В итоге оставил feature_fraction_by_node == 0.6 (использовал как дефолтное значение при подборе признаков и т. д.), остальные параметры не менял.
...
4. Попробовать другие подходы к моделированию. Например, можно было сначала понизить размерность признакового пространства автокодировщиком и только потом применять модель. Или применить сначала шумоподавляющий автокодировщик.
